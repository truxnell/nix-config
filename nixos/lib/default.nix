  ...
  mkService =
    ...
    (
      let
        ...
        addTraefikLabels =
          if
            (builtins.hasAttr "container" options) && (builtins.hasAttr "addTraefikLabels" options.container)
          then
            options.container.addTraefikLabels
          else
            true;
        ...
        containerExtraOptions =
          ...
          ++ lib.optionals (lib.attrsets.attrByPath [ "container" "caps" "tmpfs" ] false options) [
            (map (folders: "--tmpfs=${folders}") container.caps.tmpfsFolders)
          ]
  ...
  mkTraefikLabels =
    ...
    (
      let
        ...
        middleware =
          if builtins.hasAttr "middleware" options then options.middleware else "local-ip-only@file";
      in
      {
        "traefik.enable" = "true";
