...
let
  ...
  miniflux-reset-feed-errors = pkgs.writeShellScriptBin "miniflux-reset-feed-errors" ''
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    warnings = [
      ...
      (mkIf (!config.services.postgresql.enable) "WARNING: Postgres is not enabled on host for ${app}!")
    ];
