...
${if localBackups != [] then lib.concatMapStrings (name: "    sudo systemctl start restic-backups-${name}.service\n") localBackups else "    echo 'No local backups configured'\n"}

backup-remote:
    #!/usr/bin/env bash
    set -euo pipefail
${if remoteBackups != [] then lib.concatMapStrings (name: "    sudo systemctl start restic-backups-${name}.service\n") remoteBackups else "    echo 'No remote backups configured'\n"}
