...
let
  ...
  image = "factoriotools/factorio:stable@sha256:9e8e7fe91c97fca437c00b447db373741b45f756d65811bf2da1145c68195860";
  user = "845"; # string
  group = "845"; # string
  port = 34203; # int
  port_rcon = 27019; # int
...
in
{
  ...
  options.mySystem.services.${app}.${instance} = {
    ...
    addToHomepage = mkEnableOption "Add ${app} - ${instance} to homepage" // {
      default = true;
    };
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.oci-containers.containers."${app}-${instance}" = {
      ...
      ports = [
        "${builtins.toString port}:${builtins.toString port}/UDP"
        "${builtins.toString port_rcon}:${builtins.toString port_rcon}/UDP"
      ]; # expose port
    };
